...
describe("Python string extraction", () => {
	...
	describe("extractStringContent — quote stripping", () => {
		it("should strip double quotes", () => {
			...
			expect(pythonLanguageSupport.extractStringContent(node)).toBe("content");
		});
	...
	describe("extractStringContent — prefix handling", () => {
		it("should handle f prefix", () => {
			...
			expect(pythonLanguageSupport.extractStringContent(node)).toBe("text");
		});
		...
		it("should handle b prefix (byte string)", () => {
			...
			expect(pythonLanguageSupport.extractStringContent(node)).toBe("bytes");
		});
		...
		it("should handle rb prefix", () => {
			...
			expect(pythonLanguageSupport.extractStringContent(node)).toBe("rawbytes");
		});
	...
	describe("extractStringContent — edge cases", () => {
		...
		it("should handle string with escape sequences", () => {
			...
			expect(pythonLanguageSupport.extractStringContent(node)).toBe("line1\\nline2\\ttab");
		});

		it("should handle string with unicode", () => {
			const node = getFirstStringNode('x = "café ☕ 日本語"');
			expect(pythonLanguageSupport.extractStringContent(node)).toBe("café ☕ 日本語");
		});
    ...
    "part2"
			...
			expect(pythonLanguageSupport.extractConcatenatedString(node)).toBe("part1part2");
		});

		it("should handle mixed quote styles", () => {
			const node = getFirstConcatNode("x = (\"double\" 'single')");
			expect(pythonLanguageSupport.extractConcatenatedString(node)).toBe("doublesingle");
		});

		it("should handle f-string parts in concatenation", () => {
			const node = getFirstConcatNode('x = ("prefix " f"hello {name}")');
			expect(pythonLanguageSupport.extractConcatenatedString(node)).toBe("prefix hello {}");
		});

		it("should handle concatenation with triple-quoted parts", () => {
			const node = getFirstConcatNode('x = ("simple" """triple""")');
			expect(pythonLanguageSupport.extractConcatenatedString(node)).toBe("simpletriple");
		});
	});
});
